...
const variantClass: Record<BadgeVariant, string> = {
  ...
  'info-soft': 'text-info-text bg-info-bg border border-info-border/35',
  'secondary-blue': 'text-action-secondary bg-info-bg border border-transparent',
  warning: 'text-warning-text bg-warning-bg border border-warning-border/30',
  'success-restrained': 'text-success-text bg-success-bg border border-success-border/45',
  danger: 'text-danger-text bg-danger-bg border border-danger-border/30',
  ...
  neutral: 'text-text-secondary bg-surface-subtle border border-border-default',
}
...
export function Badge({ variant = 'neutral', icon, className, children }: BadgeProps) {
  return (
    <span
      className={cn(
        'inline-flex min-h-5 items-center gap-1 whitespace-nowrap rounded-md px-2 py-0.5',
  ...
  return (
    <span
      ...
      className={cn(
        ...
        'bg-action-primary px-1.5 text-[11px] font-semibold text-text-inverse',
        className,
      )}
    >
      {count > 99 ? '+99' : count}
