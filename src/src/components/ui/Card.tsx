...
export function Card({ padding = 'default', className, children, ...rest }: CardProps) {
  return (
    <div
      className={cn(
        'rounded-lg border border-divider-soft bg-surface shadow-xs',
  ...
  return (
    <div className={cn('mb-4 flex flex-wrap items-start justify-between gap-3 md:mb-5 md:gap-4', className)}>
      <div className="min-w-0">
        <h3 className="text-h3 font-bold tracking-[-0.015em] text-text-primary md:text-[20px] md:leading-8">{title}</h3>
        {description && (
